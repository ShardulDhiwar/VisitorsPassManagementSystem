...
export const issuePass = async (req, res) => {
    try {
        ...
        // Update visitor with new passToken
        await Visitor.findByIdAndUpdate(
            appointment.visitorId._id,
            { passToken: token }
        );
...
export const updatePassStatus = async (req, res) => {
    try {
        ...
        if (pass.status === "expired") {
            return res.status(400).json({
                success: false,
                message: "Cannot update an expired pass"
            });
        }

        const visitor = await Visitor.findById(pass.visitorId);

        // Update visitor status based on pass status change
        if (status === "used" && pass.status === "issued") {
            // Manual check-in
            visitor.isInside = true;
            visitor.entryTime = new Date();
            visitor.exitTime = null;
            ...
            await CheckLog.create({
                ...
                appointmentId: pass.appointmentId,
                action: "check-in",
                doneBy: "security-manual"
            });
        } else if (status === "expired" && pass.status === "used") {
            // Manual check-out
            visitor.isInside = false;
            visitor.exitTime = new Date();
            await visitor.save();
